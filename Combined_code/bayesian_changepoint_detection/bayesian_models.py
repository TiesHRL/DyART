...
class OnlineChagepoint():
    def __init__(self, data, hazard_function, hazard_input):
    ...
    def warm_run(self, llc, t):
        for i in range(t):
            llc.update_theta(self.data[t], t=i)
        self.R[0, t] = 1
        return llc
    def iteration(self, t, log_likelihood_class, Nw):
        x = self.data[t]
        # Evaluate the predictive distribution for the new datum under each of
        # the parameters.  This is the standard thing from Bayesian inference.
        predprobs = log_likelihood_class.pdf(x)
        # Evaluate the hazard function for this interval
        H = self.hazard_function(self.hazard_input, np.array(range(t + 1)))
        # Evaluate the growth probabilities - shift the probabilities down and to
        # the right, scaled by the hazard function and the predictive
        # probabilities.        
        ...
        # Update the parameter sets for each possible run length.
        log_likelihood_class.update_theta(x, t=t)
        run_likelihood = self.R[Nw,t]
        if run_likelihood > 0.4 and Nw != t:
            return True
        else:
            return False
