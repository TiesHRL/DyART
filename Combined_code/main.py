...
def objective_function(p, max_depth, min_size, max_weight, start, fin, splt, data):
    ...
    return performance
def optimizer(pbounds, start, fin, splt, retune_it, data, init_points=10, n_iter=30):
    optimizer = BayesianOptimization(f= lambda p, max_depth, min_size, max_weight: objective_function(p, max_depth, min_size, max_weight, start, fin, splt, data), pbounds=pbounds, random_state=1)
...
def main():
    ...
    pd.DataFrame([ARTX_exog_tuned, ARTX_exog_trained, ARTX_target_tuned, ARTX_target_trained, ARX_p_trained, ARX_p, AR_p]).to_csv(f"Data\\results_{prep_met}.csv")
